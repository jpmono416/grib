...
func ReadNMessagesOrReturnRaw(gribFile io.Reader, n int) ([]*Message, [][]float64, error) {
	...
	for {
		...
		if messageErr != nil {
			if strings.Contains(messageErr.Error(), "EOF") {
				return messages, rawData, nil
			}
			log.Println("Error when parsing a message, saved raw data. ", messageErr.Error())
			rawData = append(rawData, message.Data())
		}
		messages = append(messages, message)
		if len(messages) >= n {
			return messages, rawData, nil
...
func ReadSection5(f io.Reader, length int) (section Section5, err error) {
	...
	if !dataTemplateSupported(int(section.DataTemplateNumber)) {
...
func ReadSection7(f io.Reader, length int, section5 Section5) (section Section7, sectionError error) {
	...
	if length != 0 {
		...
		case Data40:
			section.Data, sectionError = ParseData40(f)
		default:
...
func dataTemplateSupported(template int) bool {
	supportedTemplateNumbers := []int{0, 2, 3, 40}
	for _, code := range supportedTemplateNumbers {
		if code == template {
			return true
		}
	}
